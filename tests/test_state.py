...
from bot.actions import decode, N_ACTIONS, WHACK_BASE, BUSTER_BASE, MINE_BASE, ACTION_ICE, ACTION_COLLECT_SUN, ACTION_WAIT, rc_to_cell
...
busters = [i - BUSTER_BASE for i in range(BUSTER_BASE, MINE_BASE) if mask[i]]
assert sorted(busters) == sorted([rc_to_cell(1, 3), rc_to_cell(4, 8)]), busters
# mina svuda osim grobova (2) i postojecih mina (2): 45-4=41
mines = [i for i in range(MINE_BASE, ACTION_ICE) if mask[i]]
assert len(mines) == 41, len(mines)
# ice nije spreman
assert not mask[ACTION_ICE]
assert mask[ACTION_COLLECT_SUN] and mask[ACTION_WAIT]

# grid kanali: kanta na (2,4) -> kanal 2 i hits 3/3
grid = obs[:315].reshape(7, 5, 9)
...
# mine: naoruzana 1.0, nenaoruzana 0.5
assert grid[6, 3, 1] == 1.0 and grid[6, 0, 2] == 0.5
